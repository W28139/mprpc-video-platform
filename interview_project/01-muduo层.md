...
这是学习项目**。整个 `wevix_muduo` 是一个「从零实现」的网络库，内存池是其中的一个模块。直接链接 tcmalloc 就完全跳过了「**怎么设计一个多层缓存的分配器**」这个学习过程——而这个过程里的东西（线程本地缓存怎么免锁、批量搬运怎么减少锁争用、Span 怎么管理、延迟归还怎么减少跨层操作）**才是这个模块真正的价值。
