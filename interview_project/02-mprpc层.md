...
> **怎么用这份文档**
>
> - 每道题 = **30 秒口述版**（面试现场直接说出口的话）+ **面试官想听什么**（考点）+ **可能追问**（预判下一刀）。
> - 标 🔥 的是**深挖剧本**——「面试官问 → 我答 → 面试官再追问」的对话体，模拟真实压力。
> - 代码引用以**当前仓库代码为准**（`file:line` 可直接跳转）。`mprpc/` 是框架层，被 `video_platform/` 的 5 个微服务共同使用。
> - 数据来源：**【实测】** = 本机重新验证；**【记录】** = 开发日志历史值。
...
- [一、整体设计](#一整体设计)
- [二、协议设计](#二协议设计)
- [三、拆帧：RpcMessageCodec](#三拆帧rpcmessagecodec)
- [四、客户端：MprpcChannel](#四客户端mprpcchannel)
- [五、服务端：RpcProvider](#五服务端rpcprovider)
- [六、服务注册发现：ZooKeeper](#六服务注册发现zookeeper)
- [七、连接池](#七连接池)
- [八、超时、重试与 deadline](#八超时重试与-deadline)
- [九、配置与初始化](#九配置与初始化)
- [十、可观测性：自研 metrics 库](#十可观测性自研-metrics-库)
- [🔥 深挖剧本](#-深挖剧本)
...
- 能一句话说清「protobuf 管序列化、ZK 管发现、muduo 管网络」这三者的**职责边界**
- 知道**帧边界是在网络层解决的**，不是业务层
...
**如果这是生产项目**，我会毫不犹豫用 gRPC 或 brpc——它们有完整的超时/重试/负载均衡/可观测性/多语言支持，而且经过大规模验证。**自研 RPC 框架在生产环境几乎总是错误选择**，除非团队有非常特殊的需求。
...
- 能说清「协议层不依赖传输层」这个**解耦点**，以及它的实际价值（网络库可替换）
- 知道 15 个错误码是和协议层放在一起的（因为它们属于「协议语义」而不是「业务语义」）
...
- 能说清「错误码分层」的**目的**是为了「**指导重试决策**」，不是为了好看
- 能指出只有网络层的错误进重试白名单
